---
name: recall
description: "Use when the user wants detail from a past session that is no longer in context — why something was built a certain way, what was already tried, or specifics from an earlier session. Triggers: 'recall', '예전에', '왜 이렇게 했지', '전에 뭐 시도했지', 'dig up past session'"
---

# Recall (Archived Session Detail)

Compaction replaces conversation history with a summary — specifics survive only
in the archived transcript. This skill digs them back out.
...
> Transcript archiving is written by the Claude Code hook. In Codex you can read
